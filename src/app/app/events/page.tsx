    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-muted mt-1">Rallies and meetings — check in to record your attendance.</p>
        </div>
        {admin && <EventCreateForm districts={districts} />}
      </div>

      {eventList.length === 0 ? (
        <Panel><p className="text-sm text-muted py-8 text-center">No events yet.{admin ? " Create the first one above." : ""}</p></Panel>
      ) : (
            ...
            return (
              <div key={ev.id} className="panel p-5">
                <div className="flex items-start justify-between gap-3">
                  ...
                  {isAttended && <Badge color="var(--accent)"><CheckCircle2 size={11} /> Checked in</Badge>}
                </div>
                ...
                <div className="mt-3 space-y-1.5 text-sm text-muted">
                  ...
                  <div className="flex items-center gap-2"><Users size={14} /> {counts.get(ev.id) ?? 0} checked in · {ev.expected} expected</div>
                </div>
                <div className="flex items-center gap-2 mt-4">
                  ...
                  {isAttended && <span className="text-sm text-[var(--accent)]">✓ Attendance recorded</span>}
