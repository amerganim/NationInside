...
function Submit() {
  ...
  return (
    <button disabled={pending} className="rounded-lg px-4 py-2 text-sm font-medium text-white disabled:opacity-60"
      ...
      {pending ? t("form.creating") : t("taskForm.submit")}
    </button>
  );
}
  ...
  return (
    <div className="panel p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-semibold">{t("taskForm.heading")}</h3>
      ...
      <form action={async (fd) => { await createTask(fd); setOpen(false); }} className="grid sm:grid-cols-2 gap-3">
        <L label={t("form.title")} className="sm:col-span-2"><input name="title" required placeholder="Tree plantation drive" className="inp" /></L>
        <L label={t("form.category")}>
          <select name="category" className="inp">{CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}</select>
        </L>
        <L label={t("form.points")}><input name="points" type="number" min="1" defaultValue="20" className="inp" /></L>
        <L label={t("form.dueDate")}><input name="due_at" type="date" className="inp" /></L>
