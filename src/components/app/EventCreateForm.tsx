...
function Submit() {
  ...
  return (
    <button disabled={pending} className="rounded-lg px-4 py-2 text-sm font-medium text-white disabled:opacity-60"
      ...
      {pending ? t("form.creating") : t("eventForm.heading")}
    </button>
  );
}
  ...
  return (
    <div className="panel p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-semibold">{t("eventForm.heading")}</h3>
      ...
      <form action={async (fd) => { await createEvent(fd); setOpen(false); }} className="grid sm:grid-cols-2 gap-3">
        <L label={t("form.title")} className="sm:col-span-2"><input name="title" required placeholder="National Youth Rally 2026" className="inp" /></L>
        <L label={t("form.dateTime")}><input name="starts_at" type="datetime-local" required className="inp" /></L>
        <L label={t("form.expectedAttendance")}><input name="expected" type="number" min="0" defaultValue="100" className="inp" /></L>
        <L label={t("form.location")}><input name="location" placeholder="Suhrawardy Udyan, Dhaka" className="inp" /></L>
        <L label={t("form.committee")}>
          <select name="org_node_id" className="inp">
            <option value="national">{t("form.national")}</option>
            {districts.map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}
          </select>
        </L>
        <L label={t("form.description")} className="sm:col-span-2"><textarea name="description" rows={2} className="inp" /></L>
